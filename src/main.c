...
void kernel_main(bloader_boot_info_t* boot_info) {
    ...
    if (ahci_dev != 0x0) {
                                    // test reading more sectors than available (qemu)
        char* buffer = pmm_alloc((0x800000 + AHCI_SECTOR_SIZE -1) / FRAME_SIZE + 1);
        uint64_t res = ahci_read(0, 0x800000 + AHCI_SECTOR_SIZE, buffer, ahci_dev);
